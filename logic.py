...
class TicTacToe:
    ...
    def record_winner(self, winner, first_move):
        with open(self.LOG_FILE, mode='a', newline='') as file:
            writer = csv.writer(file)
            # Check if the file is empty and write the header row if it is
            if file.tell() == 0:
                writer.writerow(['Winner', 'Player1', 'Player2', 'MoveCount', 'FirstMove', 'Outcome'])
            writer.writerow([winner, 
                             'X' if self.player1_type == 'human' else 'Bot',
                             'O' if self.player2_type == 'human' else 'Bot',
                             sum(1 for row in self.board for cell in row if cell),
                             first_move,
                             'Win' if winner == 'X' else ('Draw' if winner == 'Draw' else 'Lose')
                            ])